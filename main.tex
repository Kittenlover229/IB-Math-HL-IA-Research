...
cell we might need is 1 respectively. After that we can apply the A* pathfinding
...
$\psi_i(x_j) = 0, j \neq i$. So for the example set $A$ the fit function is 
...
\end{equation}
\begin{equation}
f(x) = \sum_{i=1}^{|A|} \left(y_i * \prod_{j=0, j \neq i}^{|A|} \frac{x - x_j}{x_i - x_j}\right)
\end{equation}
...
some $a_i$. In this case the pathfinding algorithm requires the path to turn back,
...
$f(x)$ can't be a well-defined function, since that requires it to have to
